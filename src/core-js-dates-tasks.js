...
function getNextFriday(date) {
  ...
  if (currentDay < target) {
    const sum = (target - currentDay) * oneDay;
    result = new Date(currentDate + sum).toISOString();
  }
  if (currentDay >= target) {
    const sum = (7 - currentDay + target) * oneDay;
    result = new Date(currentDate + sum).toISOString();
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const oneDay = 24 * 60 * 60 * 1000;
  return (Date.parse(dateEnd) - Date.parse(dateStart)) / oneDay + 1;
}
...
function formatDate(date) {
  ...
  return `${month + 1}/${day}/${year}, ${hours}:${minutes}:${seconds} ${period}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= lastDay; i += 1) {
    const firstDay = new Date(year, month - 1, i);
    arr.push(firstDay);
  }
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (String(arr[i]).startsWith('Sun')) {
      count += 1;
    }
    if (String(arr[i]).startsWith('Sat')) {
      count += 1;
    }
  }
